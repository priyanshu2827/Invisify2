...
const Hyperspeed = ({ effectOptions: customOptions }: HyperspeedProps) => {
  const hyperspeed = useRef<HTMLDivElement>(null);
  const appRef = useRef<any>(null);

  const effectOptions = { ...hyperspeedPresets.one, ...customOptions };
  ...
  useEffect(() => {
    ...
    class CarLights {
      mesh: THREE.Mesh;
      webgl: App;
      options: typeof effectOptions;
      colors: number[] | THREE.Color;
      speed: number[];
      fade: THREE.Vector2;

      constructor(webgl: App, options: typeof effectOptions, colors: number[] | THREE.Color, speed: number[], fade: THREE.Vector2) {
        this.webgl = webgl;
    ...
    class LightsSticks {
      ...
      constructor(webgl: App, options: typeof effectOptions) {
        this.webgl = webgl;
        this.options = options;
        this.mesh = new THREE.Mesh();
      }
    ...
    class Road {
      ...
      createPlane(side: number, width: number, isRoad: boolean) {
        ...
        let roadMarkings_fragment = `
          ...
          float brokenLinesWidth = 1./uLanes * uBrokenLinesWidthPercentage;
          float brokenLines = 0.;
        ...
        const material = new THREE.ShaderMaterial({
          fragmentShader: roadBaseFragment,
    ...
    function resizeRendererToDisplaySize(renderer: THREE.WebGLRenderer, setSize: App['setSize']) {
